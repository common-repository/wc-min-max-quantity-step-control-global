...
function wcmmq_g_min_max_valitaion( $bool, $product_id, $quantity, $variation_id = 0, $variations = false ){ //Right two parameters added
     ...
     */
    $current_qty_inCart = wcmmq_g_check_quantity_in_cart( $product_id,$variation_id );
    ...
    if( $total_quantity <= $max_quantity && $total_quantity >= $min_quantity  ){
        ...
        if( $current_qty_inCart > 0 ){
            $message .= sprintf( WC_MMQ_G::getOption( '_wcmmq_g_msg_max_limit_with_already' ), $current_qty_inCart, $product_name );
            $message .= " <br>";
        }
...
add_filter('woocommerce_add_to_cart_validation', 'wcmmq_g_min_max_valitaion', 10, 5); //When add to cart
 ...
 * @since 1.0
 */
function wcmmq_g_update_cart_validation( $true, $cart_item_key, $values, $quantity ) { 
...
add_filter('woocommerce_update_cart_validation', 'wcmmq_g_update_cart_validation', 10, 4); //When Update cart
